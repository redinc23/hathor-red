...
- **clearQueue clears Media Session (dose-1.111)**: emptying the queue nulls `navigator.mediaSession.metadata` and sets playbackState to `none` (same honesty as logout path) so lock-screen / OS controls do not keep a stale track
...
Dose 1.111 clearQueue Media Session honesty closed. Dose 1 polish largely complete; next Dose 2/4 leftovers (profile polish, room edge cases). No Dose 6+.
