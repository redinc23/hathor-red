...
def generate_markdown_report(db: Session) -> str:
    ...
    risk_rows = [
        [
            ...
            r.impact.value,
            r.mitigation,
            r.evidence_needed or "Unspecified",
        ]
        for r in risks
    ]

    inv_sections = []
    inv_sections.append("## Current-State Map (Components)\n")
    inv_sections.append(
        _md_table(
            ["Component", "Type", "Status", "Owner", "Evidence"],
            comp_rows or [["Unspecified", "Unspecified", "Unknown", "Unspecified", "Unspecified"]],
        )
    )
    inv_sections.append("\n## Inventory\n")
    inv_sections.append("\n### Features\n")
    inv_sections.append(
        _md_table(
            ["Name", "Owner", "Status", "User journey", "Description"],
            [
                [f.name, f.owner, f.status.value, f.user_journey or "Unspecified", f.description or ""]
                for f in features
            ]
            or [["Unspecified", "Unspecified", "Unknown", "Unspecified", ""]],
        )
    )
    inv_sections.append("\n### Integrations\n")
    inv_sections.append(
        _md_table(
            ["System", "Direction", "Protocol", "Auth", "SLA", "Status"],
            [[i.system_name, i.direction, i.protocol, i.auth, i.sla, i.status.value] for i in integrations]
            or [["Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unknown"]],
        )
    )
    inv_sections.append("\n### Environments\n")
    inv_sections.append(
        _md_table(
            ["Name", "URL", "Region", "Deploy method", "Version", "Dataset", "Status"],
            [[e.name, e.url, e.region, e.deploy_method, e.version, e.dataset, e.status.value] for e in envs]
            or [["Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unknown"]],
        )
    )
    inv_sections.append("\n### APIs\n")
    inv_sections.append(
        _md_table(
            ["Name", "Base URL", "Versioning", "Auth", "OpenAPI", "Rate limits"],
            [[a.name, a.base_url, a.versioning, a.auth, a.openapi_url, a.rate_limits] for a in apis]
            or [["Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unspecified"]],
        )
    )
    inv_sections.append("\n### Data stores\n")
    inv_sections.append(
        _md_table(
            ["Name", "Type", "Schema version", "Sensitivity", "Migrations notes"],
            [[d.name, d.db_type, d.schema_version, d.sensitivity, d.migrations_notes] for d in stores]
            or [["Unspecified", "Unspecified", "Unspecified", "Unspecified", ""]],
        )
    )
    inv_sections.append("\n### Dependencies (SBOM inputs)\n")
    inv_sections.append(
        _md_table(
            ["Name", "Version", "SBOM format", "Critical CVEs", "Owner"],
            [[d.name, d.version, d.sbom_format, d.critical_cves, d.owner] for d in deps]
            or [["Unspecified", "Unspecified", "Unspecified", "Unspecified", "Unspecified"]],
        )
    )
    inv_sections.append("\n## Backlog (WSJF)\n")
    inv_sections.append(
        _md_table(
            ["ID", "Title", "Type", "Business value", "Cost of delay", "Job size", "WSJF"],
            backlog_rows or [["", "Unspecified", "Bug", "0", "0", "1", "0.00"]],
        )
    )
    inv_sections.append("\n## Risk register\n")
    inv_sections.append(
        _md_table(
            ["ID", "Risk", "Likelihood", "Impact", "Mitigation", "Evidence needed"],
            risk_rows or [["", "Unspecified", "Medium", "Medium", "Unspecified", "Unspecified"]],
        )
    )
