...
def api_list_evidence(db: Session = Depends(get_db)) -> list[EvidenceOut]:
    return [
        EvidenceOut(id=ev.id, title=ev.title, url_or_ref=ev.url_or_ref, notes=ev.notes)  # type: ignore[arg-type]
        for ev in list_evidence(db)
    ]
...
def api_list_components(db: Session = Depends(get_db)) -> list[ComponentOut]:
    return [
        ...
        for c in list_components(db)
    ]


@router.put("/components/{component_id}/evidence", dependencies=[Depends(require("component:write"))])
def api_attach_component_evidence(
    component_id: int, payload: ComponentAttachEvidenceIn, db: Session = Depends(get_db)
) -> dict[str, str]:
    try:
